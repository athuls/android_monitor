...
//System.out.println("   die -->"+channel.getID()+",retry="+channel.retry);
  ...
  public void send(Message message, ActorReference target) {
    ...
    //System.err.println("[Custom] WWCTransportService send target and message are " + targetHost + " " + message);
    ...
    synchronized (this){
      ...
      if (channel != null) {channel.put(message);}
    	...
    	//System.err.println("[Custom] WWCTransportService send target and message newly created are " + targetHost + " " + message);
        channel = (OutgoingSocketHandler) socketTable.get(targetHost);
