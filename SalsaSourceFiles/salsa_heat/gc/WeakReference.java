...
public class WeakReference implements ActorReference, java.io.Serializable{
  ...
  public void send(Message message) {
    ...
    if (target instanceof Actor) {
	    // System.err.println("[Custom] sending to weak target as actor on local " + target + " " + message);
            try {
	    ...
	    try{
		    // System.err.println("[Custom] sending to weak target as actor on remote " + target + " " + message);
       		    this.ual = (UAL)target;
	            transportService.send(message,this);
		} catch (Exception e) {
			// System.err.println("[Custom] error while putting message in transport service " + e.toString());
		}
    } else {
